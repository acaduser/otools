...
* alignmentText
	...
	プロパティパレットの鈍い反応の相手をすることなくキーストロークだけで完結できる。
...
* grip
	...
	オブジェクトをグリップ状態にする。
	空の改行で直前のオブジェクトの選択となる。
	pselectコマンドとは使い勝手が違う。
...
5. layPickNestOn
	...
	選択した単一オブジェクトの画層を表示状態にする。

6. layPickNestOff

	選択した単一オブジェクトの画層を非表示状態にする。
	外部参照のクリップ境界選択時エラーが出るどうでもいい。
